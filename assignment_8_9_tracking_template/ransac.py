...
def hdist(H: torch.Tensor, pts_matches: torch.Tensor):
    ...
    H_vec[:, 0] /= H_vec[:, 2]
    H_vec[:, 1] /= H_vec[:, 2]
    dist = torch.sum(torch.square(vec_ - (H_vec[..., :2])), dim=-1)
...
def are_any_three_collinear(points):
...
def getH(min_sample):
    ...
    if are_any_three_collinear(min_sample[:,:2]) or are_any_three_collinear(min_sample[:,2:]):
        return None
    C = torch.zeros(8, 9)
    for i in range(4):
